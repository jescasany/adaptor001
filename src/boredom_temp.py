...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 19:49:36 2017
...
class WeightBoredomHandler(BoredomHandler):
    ...
    def interaction_total_weight(self, INTERACTIONS, interaction):
        ...
        if isinstance(interaction, model.interaction.CompositeInteraction):
            return (
                INTERACTIONS.get_weight(interaction) 
                + self.interaction_total_weight(INTERACTIONS, interaction.get_pre()) 
                + self.interaction_total_weight(INTERACTIONS, interaction.get_post())
            )
        else:
            return INTERACTIONS.get_weight(interaction)
    
    def process_boredom(self, INTERACTIONS, interaction, unmodified_valence):
        if unmodified_valence > 0:
            sum = INTERACTIONS.get_total_weight()
...
class RepetitiveBoredomHandler(BoredomHandler):
    ...
    def process_boredom(self, INTERACTIONS, interaction, unmodified_valence):
        history = INTERACTIONS.get_interaction_history()[-self.HISTORY_CONSIDER_SIZE:]
