...
def lines_callback(lines_msg):
    ...
    # we need to adjust is to negate all of the m angles for each extracted
    ...
    # above in rviz.  Note that we create a new line and negate the m back
    ...
    if line != None:
         fo = follow_offset
         fa = follow_advance
         goalx = (line.c - fo) * cos(line.m) + fa * cos(line.m + pi/2)
         goaly = (line.c - fo) * sin(line.m) + fa * sin(line.m + pi/2)
