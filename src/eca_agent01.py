...
class Decode:
    ...
    def get_translation(self):
        ...
        self.raw = self.raw.replace('e1r1','move forward')
        ...
        self.raw = self.raw.replace('e5r6','right sensing')
        self.raw = self.raw.replace('e5r8','nothing on right')
        self.raw = self.raw.replace('e6r7','left sensing')
        self.raw = self.raw.replace('e6r9','nothing on left')
        return self.raw
...
#                                  "nothing on left": ("e6", "r9", 5)
        ...
        ECAAGENT01.add_child(START_STEP)
        ECAAGENT01.add_child(I_F_IS_VISITED)
        
        # Display the tree before beginning execution
        print bcolors.HEADER + "ECAAGENT01 Behavior Tree" + bcolors.ENDC
        print_tree(ECAAGENT01, indent=0, use_symbols=True)
